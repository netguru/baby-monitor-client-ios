...
enum Constants {
    ...
    /// A limit per which one notification about cry detection can be sent (in seconds).
    static let cryingNotificationRequestTimeLimit: Int = 3 * 60

    /// A limit per which noise detection notification can be sent (in seconds).
    static let noiseNotificationRequestTimeLimit: Int = 15

    /// A time after which the video stream should be hidden (in seconds).
    ...
    /// A limit for sending maximum one sound to an ML model for recognizing in this time (in seconds).
    ...
    /// A limit for which should get a confirmation id back (in seconds).
